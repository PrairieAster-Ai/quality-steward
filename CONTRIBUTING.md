# Contributing to quality-butler

Thanks for helping improve the butler. This repo is small and opinionated; the sections below
cover how it's laid out, how to test a change, and the conventions a pull request is expected to
follow. For what the butler *does*, start with the [README](README.md); for how it works,
[Technical](https://github.com/PrairieAster-Ai/quality-butler/wiki/Technical).

## Repository layout

```
agents/
  quality-butler.md     the agent definition — the behavioral contract (canonical source)
  quality-butler.yml    the GitHub Actions workflow — the only file a consuming repo copies
skills/
  ...
  github-wiki/           wiki + projects plumbing
...
- **Behavior of the agent** — edit `agents/quality-butler.md` (the prompt is the contract) and,
  if the run mechanics change, `agents/quality-butler.yml`.
...
2. Invoke it by its slash command in an interactive session — e.g. `/code-health`,
   `/security-audit`, `/code-readability assess`. The [feature reference](https://github.com/PrairieAster-Ai/quality-butler/wiki/Features) lists
   every mode and flag.
...
```bash
cp agents/quality-butler.md .claude/agents/quality-butler.md   # in the target repo
claude --agent quality-butler
```

The `verify` path in [Installation](https://github.com/PrairieAster-Ai/quality-butler/wiki/Installation) is the fastest end-to-end smoke test once the
...
```yaml
env:
  SKILLS_REF: <full commit SHA>   # pinned to a reviewed quality-butler commit
```

Two consequences for contributors:

- **`SKILLS_REF` is always a full commit SHA, never a moving branch.** The butler runs with
...
chore(butler): bump SKILLS_REF to <sha>
```
...
The butler is a composition, not a monolith, so a new capability usually arrives as a new skill
...
3. Wire it in: add it to the install loop in `agents/quality-butler.yml`, to the composed-skill
   table in `agents/quality-butler.md`, and to [Features](https://github.com/PrairieAster-Ai/quality-butler/wiki/Features) and
   [Technical](https://github.com/PrairieAster-Ai/quality-butler/wiki/Technical).
...
quality-butler reviews quality-butler. The agent can run on this repo's own pull requests — a
differential `code-review` + `security-audit` pass, with safe mechanical fixes offered as a
`butler/auto-fix-*` PR and anything non-trivial raised as a comment or issue. Treat its review
